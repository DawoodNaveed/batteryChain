...
class RecyclerAdmin extends AbstractAdmin
{
    protected function configureFormFields(FormMapper $form): void
    {
        ...
        if ($recycler->getId() === null &&
            in_array(RoleEnum::ROLE_SUPER_ADMIN, $user->getRoles(), true)
            || in_array(RoleEnum::ROLE_ADMIN, $user->getRoles(), true)) {
            $manufacturers = $this->manufacturerService->toChoiceArray(
                $this->manufacturerService->manufacturerRepository->findAll()
            );
            $form
                ->add('isFallback', ChoiceFieldMaskType::class, [
                    'mapped' => false,
                    'choices' => [
                        'Yes' => 'yes',
                        'No' => 'no',
    ...
    protected function configureDatagridFilters(DatagridMapper $filter): void
    {
        ...
        if (in_array(RoleEnum::ROLE_SUPER_ADMIN, $user->getRoles(), true)
            || in_array(RoleEnum::ROLE_ADMIN, $user->getRoles(), true)) {
            $filter->add('isFallback', null, [ 'label' => "Fallback" ]);
        }
    }
    ...
    protected function prePersist(object $object): void
    {
        ...
        if (in_array(RoleEnum::ROLE_SUPER_ADMIN, $user->getRoles(), true)
            || in_array(RoleEnum::ROLE_ADMIN, $user->getRoles(), true)) {
            if ($this->getForm()->get('isFallback')->getData() === 'yes') {
                $object->setIsFallback(true);
            } else {
                $object->setIsFallback(false);
                $object->addManufacturer($this->getForm()->get('manufacturers')->getData());
            }
        }
